...
self.asignaCategoria = async function (req, res) {
    try{
        let itemToAssing = await categoria.findByPk(req.body.categoriaid);
        if(!itemToAssing) return res.status(404).send()

        let item = await pelicula.findByPk(req.params.id);
        if(!item) return res.status(404).send()
        
        await item.addCategoria(itemToAssing)
        return res.status(204).send()
    }catch(error){
        return res.status(500).json(error)
    }
...
// DELETE: api/pelicula/5/categoria/1
